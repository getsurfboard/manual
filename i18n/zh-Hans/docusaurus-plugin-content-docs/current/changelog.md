...
### mobile-2.34.4
- **新功能**
  - **置顶配置快捷方式**：置顶的配置现在会显示为应用图标的动态快捷方式，长按图标即可快速切换。
  - Hysteria2：支持 `gecko-password` 参数。
  - VMess：支持 `encrypt-method` 参数。
  - MTU 上限从 32000 提高到 64000。
- **修复**
  - 修复通过深层链接启动 VPN 时的界面闪烁。
  - 修复 WireGuard 端点为 IPv6 地址时无法连接的问题。
  - 修复停止后立即重启 VPN 偶发崩溃的问题。
  - 修复切换配置后偶尔回退到之前选中项的问题。
  - 系统阻止 VPN 自动启动时改为提示而不是崩溃。
  - 修复无法弹出 VPN 权限对话框时崩溃的问题。
  - 修复打开文件选择器时崩溃的问题。
  - 修复仪表盘动画过程中偶发崩溃的问题。
  - 修复导出备份导致 VPN 断开的问题。
  - 修复开发者选项中「上次退出原因」时间戳的换行显示问题。
- **改进**
  - 减小应用体积。

### mobile-2.34.3
- **修复**
  - 修复开启 SSH Dump 时应用崩溃的问题。
  - 修复在没有文件管理器的设备（如部分 TV 盒子）上打开文件选择器时崩溃的问题。

### mobile-2.34.2
- **新功能**
  - **默认配置**：全新安装现在自带一份开箱即用的默认配置，内置广告拦截规则。
- **修复**
  - 修复使用三键导航时 Snackbar 悬浮在底部导航栏上方的问题。
  - 修复 IPv6 不可达时连接一直卡住的问题，现在会快速失败。
  - 修复 DOMAIN-SET / RULE-SET 列表内容变化后规则不更新的问题。
  - 修复原生引擎的一处稳定性问题。
- **改进**
  - 提升 TCP/UDP 转发性能，降低每包 CPU 开销。
  - 网速图表更流畅，消除闪烁。
  - 更新内置 GeoIP 数据库。

### mobile-2.34.1
- **新功能**
  - **备份与恢复**：可将应用设置和代理配置文件导出为带密码保护的归档文件，并在其他设备上恢复，支持跨版本兼容。
  - **子网代理组**：支持 `subnet` 代理组类型，可根据当前网络环境（如 WiFi、蜂窝网络、SSID、网关）自动选择代理。
  - **mDNS 解析**：为第三方应用解析 `.local` 域名；多播流量现在直接发送。
  - 返回应用或切换标签页时，仪表盘会自动刷新公网 IP 和 DNS 信息。
- **修复**
  - 修复内置 GeoIP 数据库文件损坏时崩溃的问题。
  - 修复 VPN 停止后更换代理组时崩溃的问题。
  - 修复 Activity 重建过程中切换 Tab 时崩溃的问题。
  - 代理信息弹窗保持系统栏全面屏显示。
  - 修复 VPN 启动失败时界面卡死的问题。
- **改进**
  - 改进网速图表显示。
...
- **改进**
  - 优化配置导入速度。
  - 测速前增加预热请求，减少冷启动对测速准确性的影响。
- **修复**
  - 修复大型基于域名的规则集（RULE-SET）不生效的问题。
  - 修复配置文件行尾注释解析错误的问题。
  - 修复 DOMAIN-SET 和嵌套 RULE-SET 规则数量显示严重偏低的问题。
  - 修复部分 HTTPS 代理服务器的兼容性问题。
...
- **新功能**
  - **底层代理（代理链）**: 各代理类型现支持配置底层代理，实现代理链功能。
  - **画中画网速悬浮窗**: 实时网速可通过画中画悬浮窗显示。（需要 Android 8+）
  - **配置编辑器键盘快捷键**: 新增 Readline 风格键盘快捷键，支持 Ctrl+T 字符转置、Ctrl+A/Ctrl+E 光标跳转等。
  - **外部资源 HTTP 基本认证**: 外部资源下载 URL 支持通过 userinfo 进行 HTTP 基本认证。

- **修复**
  - 修复配置文件下载请求的 Cookie 持久化问题。
  - 修复编辑器中打开超大配置文件时的崩溃问题。
  - 修复删除后编辑 DoH 服务器时的崩溃问题。
  - 修复 STUN NAT 检测的可靠性问题。
  - 修复 LogcatViewer 在分屏模式下的显示问题。

- **改进**
  - 通过 C++ SuffixTree 实现优化域名匹配性能。
  - 减少不必要的数据解压缩，提升配置文件加载速度。
  - 减少系统监控面板的内存消耗。
  - 更新内置 GeoIP 数据库。
...
- **新功能**
  - **降低 MTU 下限**: MTU 现在可以最低设置为 1400。
  - **Host 列表支持 DoH**: Host 列表现在支持添加 DoH 服务器条目。
...
- **新功能**
  - **Host 规则支持 DoH 服务器**: 现在可以在配置文件的 `[Host]` 段中配置 DNS over HTTPS 服务器。
  - **代理组图标 URL 解析**: 代理组现支持解析 `icon-url` 参数（界面显示暂不支持）。
  - **引号包裹的代理名**: 代理名被引号包裹时，现可在查找、验证和界面显示中完全支持。

- **修复**
  - 修复由真实 IP 查询导致的 DNS 泄露问题。
  - 修复 Volley 队列瓶颈导致的外部资源下载缓慢问题。
...
- **新功能**
  - **Smart 代理组**: 支持导入包含 `smart` 代理组类型的 Surge 配置文件（暂时作为 `url-test`/`auto` 类型兼容处理）。
  - **DNS Host 规则增强**: Host 规则现可对代理服务器域名生效（之前仅对目标域名生效）。
  - **自定义 DNS 端口**: DNS server host rule 现支持自定义端口号。

- **修复**
  - 修复流量列表在数据更新时自动滚动的问题。

- **改进**
  - **性能**: 修复之前版本引入的吞吐率下降问题，并进一步优化网络性能。

### mobile-2.29.2

- **新功能**
  - **USER-AGENT 规则**: 支持根据 User-Agent 请求头路由 HTTP 流量，可使用通配符匹配。需要 Android 10+ 并开启 VPN HTTP 代理。
  - **仪表盘交互**: 点击网速卡片按速度排序展示流量列表；点击流量使用卡片按总流量排序展示流量列表。
  - **VPN HTTP 代理开关**: 新增设置选项用于启用/禁用 HTTP 代理功能（仅 Android 10+）。
  - **最近请求快捷入口**: 网速监控列表支持直接查看请求详情。
- **UI 与主题**
  - 全新 Material3 主题设计。
  - 修复主题切换偶尔失效的问题。
  - 修复外部资源菜单打开效果。
  - SSHDump 设置跳转增加自动滚动和高亮效果。
- **修复**
  - 修复主题设置状态丢失的问题。
  - 提升 Snell 协议兼容性。
  - 优化配置文件保存逻辑，部分解析失败时仍可保存。
...
- **新功能**
  ...
  - Logcat 和最近请求 UI 现在支持显示解码后的 IDN 域名。
